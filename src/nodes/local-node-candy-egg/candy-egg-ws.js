...
export default function(RED) {
  class WebSocketListener {
    ...
    startconn(url) {
      ...
      prefix +=
        encodeURIComponent(conf.loginUser) +
        ':' +
        encodeURIComponent(conf.loginPassword) +
        '@';
    ...
    handleConnection(/*socket*/ socket) {
        ...
        if (
          ...
          if (res.headers.location) {
            if (this.redirect > 3) {
              ...
              this.emit2all(
                'log-error',
                RED._('candy-egg-ws.errors.too-many-redirects', {
                  path: this.path,
          ...
          this.emit2all(
            'log-error',
            RED._('candy-egg-ws.errors.wrong-path', { path: this.path })
          );
        } else if (res.statusCode === 401) {
          this.emit2all(
            'log-error',
            RED._('candy-egg-ws.errors.auth-error', {
              path: this.path,
              user: this.accountConfig.loginUser
            })
          );
          ++this.authRetry;
          ...
          this.emit2all(
            'log-error',
            RED._('candy-egg-ws.errors.server-error', {
              path: this.path,
      ...
      socket.on('error', err => {
        ...
        this.emit2all(
          'log-error',
          RED._('candy-egg-ws.errors.connect-error', {
            err: err,
    ...
    broadcast(data) {
      try {
        ...
        this.emit2all(
          'log-error',
          RED._('candy-egg-ws.errors.send-error', {
            err: err,
            accountFqn: this.accountConfig.accountFqn
          })
        );
  ...
  class WebSocketListeners {
    ...
    get(node, options = null) {
      ...
      if (
        !node.accountConfig.accountFqn ||
        !node.accountConfig.loginUser ||
        !node.accountConfig.loginPassword
      ) {
        throw new Error(RED._('candy-egg-ws.errors.missing-conf'));
      }
  ...
  class CANDYEggAccountNode {
    constructor(n) {
      ...
      this.accountFqn = this.credentials
        ? this.credentials.accountFqn
        : n.accountFqn;
      this.loginUser = this.credentials
        ? this.credentials.loginUser
        : n.loginUser;
      this.loginPassword = this.credentials
